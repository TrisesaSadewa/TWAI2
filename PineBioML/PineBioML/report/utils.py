...
class basic_plot(ABC):
    ...
    def save_name(self):
        ...
        return "{}{}.png".format(self.prefix, self.name)
...
def classification_summary(y_true,
    ...
    try:
        ...
        artifacts = {}
        for f in glob.glob(save_path + prefix + "*.png") + glob.glob(save_path + prefix + "*.csv"):
            key = os.path.basename(f).replace(prefix, "").replace(" ", "_").strip("_").lower()
            if key.endswith(".png"): key = key[:-4]
            if key.endswith(".csv"): key = key[:-4] + "_csv"
            artifacts[key] = os.path.abspath(f)
        if artifacts:
            requests.post(
                os.environ.get("FASTAPI_EXPORT_URL", "http://127.0.0.1:8000/export/update-artifacts"),
                json={"job_id": os.environ.get("PINEBIOML_JOB_ID", prefix.strip()), "artifacts": artifacts},
                timeout=5
            )
    except Exception:
        pass
...
# ── New Missing Artifact Generators ──────────────────────────────
class feature_importance_plot(basic_plot):
    ...
    def draw(self, model, feature_names):
        importances = None
        ...
        if importances is not None:
            ...
            plt.title(f'{self.prefix} Feature Importance')
            plt.tight_layout()

class shap_summary_plot(basic_plot):
    def __init__(self, prefix="", save_path="./output/images/", save_fig=True, show_fig=True):
        super().__init__(prefix=prefix, save_path=save_path, save_fig=save_fig, show_fig=show_fig)
        self.name = "shap_summary"
        
    def draw(self, explainer, x):
        try:
            import shap
            shap_values = explainer.shap_values(x)
            plt.figure()
            vals = shap_values[1] if isinstance(shap_values, list) else shap_values
            shap.summary_plot(vals, pd.DataFrame(x), show=False)
        except Exception as e:
            print(f"SHAP plot failed: {e}")

class learning_curve_plot(basic_plot):
    def __init__(self, prefix="", save_path="./output/images/", save_fig=True, show_fig=True):
        super().__init__(prefix=prefix, save_path=save_path, save_fig=save_fig, show_fig=show_fig)
    ...
    def draw(self, model, *args, **kwargs):
        if hasattr(model, 'loss_curve_'):
            ...
            plt.title(f'{self.prefix} Learning Curve')
            plt.xlabel('Epochs')
