...
class ReportEngine:
    ...
    def _render_model_performance_table(self, all_models: list, metrics: dict = None, task_type: str = "") -> str:
        ...
        def fmt_val(label, val):
            """Format a cell value: probabilities as %, MCC as decimal."""
            if val is None:
                return "N/A"
            if isinstance(val, (int, float)):
                if label == "Test MCC":
        ...
        callout_html = (
            ...
            f'<strong style="color:#f8fafc;font-size:1.05rem;font-weight:600;">{best_name}</strong>'
            f'<span style="color:#94a3b8;margin:0 10px;">&bull;</span>'
            f'<span style="color:#94a3b8;">Test Accuracy:</span> <strong style="color:#10b981;font-size:1.05rem;">{best_acc_str}</strong>'
            f'</div>'
        )
        ...
        for i, row in enumerate(all_models):
            ...
            params_key = next((k for k in row.keys() if k.lower() in ['best_params', 'params']), None)
            has_params = bool(params_key and row.get(params_key))
    ...
    def _markdown_to_html(self, text) -> str:
        ...
        for block in blocks:
            ...
            for line in lines:
                stripped = line.strip()
                is_table_row = stripped.startswith('|') and stripped.endswith('|') and len(stripped.split('|')) > 2
                if is_table_row:
                    if in_list:
                    ...
                    if not in_table:
                        in_table = True
                        processed_lines.append('<div style="overflow-x:auto; margin-bottom: 1.5rem;"><table class="model-table"><thead><tr>')
                        ...
                        cells = [c.strip() for c in stripped.split('|') if c.strip() or not stripped.startswith('|')]
                        # Ensure we don't drop empty cells if it's a true column, but since it's a basic parser, we handle it simply
                        # Just strip ends if pipes exist, otherwise split normally
            ...
            if block_html.endswith('<br>'):
                block_html = block_html[:-4]
            
            if '<ul>' not in block_html and '<table' not in block_html:
                block_html = f"<p>{block_html}</p>"
            else:
                block_html = f"<div>{block_html}</div>"
    ...
    def _artifact_url(self, report_id: str, artifact_path: str) -> str:
        ...
        try:
            ...
            if os.path.abspath(artifact_path).startswith(os.path.abspath(output_dir)):
                ...
                return f"/api/artifacts/{report_id}/{quote(filename)}"
        except Exception:
            pass
